...
/* ====================== DROPDOWNS DA TOPBAR ======================
   Padrao da skill: dropdown de usuario (perfil/preferencias/sair) + de notificacoes.
   Persistem no shell — funcionam em todas as telas. */
(function topbarDropdowns() {
  ...
  list.innerHTML = NOTIFS.map((n, i) => `
    <div class="notif-item ${i < 3 ? 'unread' : ''}" style="--c:var(${n.c})">
      <span class="notif-ic">${App.svg(n.ic)}</span>
  ...
  document.querySelector('.pop-allbtn')?.addEventListener('click', () => App.toast('info', 'Notificações', 'abrindo central de notificações'));
  ...
  document.querySelectorAll('#userDropdown .menu-flat .mi').forEach((mi) => {
    mi.addEventListener('click', () => {
      ...
      if (/sair/i.test(label)) App.toast('warn', 'Sessão encerrada', 'até logo, Rodrigo');
      else App.toast('info', label, 'ação de exemplo');
    });
  });
})();
